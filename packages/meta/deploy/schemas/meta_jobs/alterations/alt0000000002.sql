-- Deploy: schemas/meta_jobs/alterations/alt0000000002 to pg
...
ALTER DEFAULT PRIVILEGES IN SCHEMA "meta_jobs" GRANT EXECUTE ON FUNCTIONS TO administrator;
CREATE FUNCTION "meta_jobs".json_build_object_apply (arguments text[])
  RETURNS json
  AS $EOFCODE$
DECLARE
  arg text;
...
CREATE FUNCTION "meta_jobs".add_job (identifier text, payload json DEFAULT '{}' ::json, job_key text DEFAULT NULL, queue_name text DEFAULT NULL, run_at timestamptz DEFAULT now(), max_attempts int DEFAULT 25, priority int DEFAULT 0)
  RETURNS "meta_jobs".jobs
  AS $EOFCODE$
DECLARE
...
CREATE FUNCTION "meta_jobs".add_scheduled_job (identifier text, payload json DEFAULT '{}' ::json, schedule_info json DEFAULT '{}' ::json, job_key text DEFAULT NULL, queue_name text DEFAULT NULL, max_attempts int DEFAULT 25, priority int DEFAULT 0)
  RETURNS "meta_jobs".scheduled_jobs
  AS $EOFCODE$
DECLARE
...
VOLATILE
SECURITY DEFINER;
CREATE FUNCTION "meta_jobs".complete_job (worker_id text, job_id bigint)
  RETURNS "meta_jobs".jobs
  LANGUAGE plpgsql
  AS $EOFCODE$
DECLARE
...
END;
$EOFCODE$;
CREATE FUNCTION "meta_jobs".complete_jobs (job_ids bigint[])
  RETURNS SETOF "meta_jobs".jobs
  LANGUAGE sql
  AS $EOFCODE$
  DELETE FROM "meta_jobs".jobs
  ...
  RETURNING
    *;
$EOFCODE$;
CREATE FUNCTION "meta_jobs".do_notify ()
  RETURNS TRIGGER
  AS $EOFCODE$
BEGIN
  PERFORM
...
$EOFCODE$
LANGUAGE plpgsql;
CREATE FUNCTION "meta_jobs".fail_job (worker_id text, job_id bigint, error_message text)
  RETURNS "meta_jobs".jobs
  LANGUAGE plpgsql
  STRICT
  AS $EOFCODE$
...
END;
$EOFCODE$;
CREATE FUNCTION "meta_jobs".get_job (worker_id text, task_identifiers text[] DEFAULT NULL, job_expiry interval DEFAULT '4 hours')
  RETURNS "meta_jobs".jobs
  LANGUAGE plpgsql
  AS $EOFCODE$
DECLARE
...
END;
$EOFCODE$;
CREATE FUNCTION "meta_jobs".get_scheduled_job (worker_id text, task_identifiers text[] DEFAULT NULL)
  RETURNS "meta_jobs".scheduled_jobs
  LANGUAGE plpgsql
  AS $EOFCODE$
DECLARE
...
END;
$EOFCODE$;
CREATE FUNCTION "meta_jobs".permanently_fail_jobs (job_ids bigint[], error_message text DEFAULT NULL)
  RETURNS SETOF "meta_jobs".jobs
  LANGUAGE sql
  AS $EOFCODE$
  UPDATE
  ...
  RETURNING
    *;
$EOFCODE$;
CREATE FUNCTION "meta_jobs".release_jobs (worker_id text)
  RETURNS void
  AS $EOFCODE$
DECLARE
BEGIN
...
LANGUAGE plpgsql
VOLATILE;
CREATE FUNCTION "meta_jobs".release_scheduled_jobs (worker_id text, ids bigint[] DEFAULT NULL)
  RETURNS void
  AS $EOFCODE$
DECLARE
BEGIN
...
LANGUAGE plpgsql
VOLATILE;
CREATE FUNCTION "meta_jobs".reschedule_jobs (job_ids bigint[], run_at timestamptz DEFAULT NULL, priority int DEFAULT NULL, attempts int DEFAULT NULL, max_attempts int DEFAULT NULL)
  RETURNS SETOF "meta_jobs".jobs
  LANGUAGE sql
  AS $EOFCODE$
  UPDATE
  ...
  RETURNING
    *;
$EOFCODE$;
CREATE FUNCTION "meta_jobs".run_scheduled_job (id bigint, job_expiry interval DEFAULT '1 hours')
  RETURNS "meta_jobs".jobs
  AS $EOFCODE$
DECLARE
  j "meta_jobs".jobs;
...
CREATE FUNCTION "meta_jobs".tg_decrease_job_queue_count ()
  RETURNS TRIGGER
  AS $EOFCODE$
DECLARE
  v_new_job_count int;
...
CREATE TRIGGER decrease_job_queue_count_on_update
  ...
  EXECUTE PROCEDURE "meta_jobs". tg_decrease_job_queue_count ();
CREATE FUNCTION "meta_jobs".tg_increase_job_queue_count ()
  RETURNS TRIGGER
  AS $EOFCODE$
BEGIN
...
CREATE FUNCTION "meta_jobs".tg_update_timestamps ()
  RETURNS TRIGGER
  AS $EOFCODE$
BEGIN
...
CREATE FUNCTION "meta_jobs".trigger_job_with_fields ()
  RETURNS TRIGGER
  AS $EOFCODE$
DECLARE
  arg text;
...
VOLATILE
SECURITY DEFINER;
CREATE FUNCTION "meta_jobs".tg_add_job_with_row_id ()
  RETURNS TRIGGER
  AS $EOFCODE$
BEGIN
  IF (TG_OP = 'INSERT' OR TG_OP = 'UPDATE') THEN
...
CREATE FUNCTION "meta_jobs".tg_add_job_with_row ()
  RETURNS TRIGGER
  AS $EOFCODE$
BEGIN
  IF (TG_OP = 'INSERT' OR TG_OP = 'UPDATE') THEN
